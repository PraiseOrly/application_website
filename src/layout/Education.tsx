...
const categoryColors: Record<string, { bg: string; text: string; border: string }> = {
  "AI & Machine Learning": { bg: "bg-green-500/10", text: "text-green-300", border: "border-green-500/20" },
  "Cloud & Infrastructure": { bg: "bg-emerald-500/10", text: "text-emerald-300", border: "border-emerald-500/20" },
};
...
const CertCard: React.FC<{ cert: Certification; onClick: () => void; index: number }> = ({ cert, onClick, index }) => {
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -3 }}
      onClick={onClick}
      className="group cursor-pointer bg-white/[0.02] border border-white/8 hover:border-green-500/25 rounded-xl p-5 transition-all duration-300 relative overflow-hidden">
      <div className="absolute top-0 left-0 w-full h-px bg-gradient-to-r from-transparent via-green-500/35 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
      <div className="flex items-start gap-3">
        <div className="p-2 bg-green-500/10 border border-green-500/15 rounded-lg shrink-0">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="flex items-center justify-between mt-2">
            <span className={`text-[10px] px-2 py-0.5 rounded-full border font-medium ${cat.bg} ${cat.text} ${cat.border}`}>
              {cert.category}
            </span>
      ...
      <div className="absolute bottom-3 right-3 text-gray-700 group-hover:text-green-400 transition-colors duration-200">
...
const CertModal: React.FC<{ cert: Certification | null; onClose: () => void }> = ({ cert, onClose }) => {
  ...
  return (
    <motion.div
      ...
      onClick={onClose}>
      <motion.div
        initial={{ scale: 0.93, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0 }}
        transition={{ type: "spring", stiffness: 200, damping: 24 }}
        className="bg-[#040f07] border border-green-500/15 rounded-2xl p-6 max-w-4xl w-full relative shadow-2xl"
        ...
        <div className="bg-[#030a06] rounded-xl overflow-hidden border border-white/5">
...
const AllCertsModal: React.FC<{ isOpen: boolean; onClose: () => void; onSelect: (cert: Certification) => void }> = ({ isOpen, onClose, onSelect }) => {
  ...
  return (
    <motion.div
      ...
      onClick={onClose}>
      <motion.div
        initial={{ scale: 0.93, y: 24 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0 }}
        transition={{ type: "spring", stiffness: 200, damping: 24 }}
        className="bg-[#040f07] border border-green-500/15 rounded-2xl p-6 max-w-3xl w-full relative shadow-2xl max-h-[80vh] overflow-y-auto"
        onClick={(e) => e.stopPropagation()}>
        <button onClick={onClose} className="absolute top-4 right-4 p-1.5 rounded-lg text-gray-500 hover:text-white hover:bg-white/8 transition-all duration-200">
          <X className="h-5 w-5" />
        </button>
        ...
        <div className="space-y-6">
          {Object.entries(grouped).map(([category, certs]) => {
            ...
            return (
              <div key={category}>
                <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold border ${cat.bg} ${cat.text} ${cat.border} mb-3`}>
                  {category}
                </span>
                <div className="grid sm:grid-cols-2 gap-3">
                  {certs.map((cert, i) => (
                    <motion.button
                      ...
                      className="text-left p-4 bg-white/[0.02] border border-white/8 hover:border-green-500/25 rounded-xl transition-all duration-200 group">
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <ExternalLink className="h-3.5 w-3.5 text-gray-700 group-hover:text-green-400 shrink-0 transition-colors duration-200 mt-0.5" />
                      </div>
                    </motion.button>
                  ))}
                </div>
        ...
        <div className="mt-6 pt-4 border-t border-white/5 flex justify-center">
          <a
            ...
            className="inline-flex items-center gap-2 px-5 py-2.5 bg-green-500/10 border border-green-500/20 text-green-300 rounded-full text-sm font-medium hover:bg-green-500/18 transition-all duration-200">
            <ExternalLink className="h-3.5 w-3.5" />
...
export const Education = () => {
  ...
  return (
    <section
      ...
      style={{ background: 'linear-gradient(to bottom, #030a06, #041208, #030a06)' }}>

      <div className="absolute top-1/4 right-0 w-80 h-80 rounded-full blur-3xl opacity-[0.04] pointer-events-none bg-green-500" />
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="grid md:grid-cols-5 gap-6">
          ...
          <motion.div
            ...
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            className="md:col-span-2 bg-green-500/[0.03] border border-green-500/15 rounded-2xl p-8 relative overflow-hidden">
            <div className="absolute top-0 left-0 w-full h-px bg-gradient-to-r from-transparent via-green-500/45 to-transparent" />
            <div className="absolute -bottom-12 -right-12 w-40 h-40 rounded-full blur-2xl opacity-[0.06] bg-green-500 pointer-events-none" />
            ...
            <div className="flex items-start gap-4 mb-6">
              <div className="p-2.5 bg-green-500/10 border border-green-500/15 rounded-xl shrink-0">
                <GraduationCap className="h-5 w-5 text-green-400" />
              </div>
              <div>
                <h3 className="text-white font-bold text-base leading-snug">{educationData.institution}</h3>
                <p className="text-green-300/80 text-sm mt-0.5">{educationData.degree}</p>
          ...
          <motion.div
            ...
            className="md:col-span-3 bg-green-500/[0.03] border border-green-500/15 rounded-2xl p-8 relative overflow-hidden">
            <div className="absolute top-0 left-0 w-full h-px bg-gradient-to-r from-transparent via-amber-500/45 to-transparent" />
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium bg-amber-500/10 border border-amber-500/20 text-amber-300 hover:bg-amber-500/18 transition-all duration-200">
